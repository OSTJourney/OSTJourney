...
base_dir = "/home/server/songs"
covers_folder = "/home/server/static/images/covers"
db_file = "songs.db"
hashes = []
        ...
        if file.endswith(".mp3"):
            try:
                ...
                try:
                    ...
                    print(f"Erreur lors de la lecture du fichier ID pour {file}: {e}")

                # Create ID if not found
                if not file_id:
                    frame_id = b"TXXX"
                    custom_frame = eyed3.id3.frames.TextFrame(frame_id, "42id")
                    audio_file.tag.frame_set[frame_id] = custom_frame
                    for frame in audio_file.tag.user_text_frames:
                        if frame.description == "42id":
                            frame.text = str(file_count)
                            file_id = frame.text
                    audio_file.tag.save()
                    new_files += 1
                ...
                try:
                    ...
                    duration = 0
                    print(f"Erreur lors de la lecture de la durée pour {file}: {e}")
                        ...
                        for frame in frames:
                            if frame_id == b"TXXX":
                                try:
                                    ...
                                    print(f"Erreur de récupération des tags TXXX pour {file}: {e}")
                            else:
                                try:
